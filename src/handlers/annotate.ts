...
async function handleSimilarIssuesAndComments(
  ...
  const footnoteRegex = /\[\^(similarity-\d+)\^\]/g;
  ...
  issueList.forEach((issue, index) => {
    ...
    const footnoteRef = `[^similarity-${footnoteIndex}^]`;
...
export function checkIfAnnotateFootNoteExists(content: string): boolean {
  const footnoteDefRegex = /\[\^(similarity-\d+)\^\]: \d+% similar to (issue|comment): [^\n]+(\n|$)/g;
...
export function removeAnnotateFootnotes(content: string): string {
  ...
  if (footnotes) {
    footnotes.forEach((footnote) => {
      const footnoteNumber = footnote.match(/similarity-\d+/)?.[0];
      if (footnoteNumber) {
