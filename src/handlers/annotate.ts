...
import { createFootnoteRef, nextFootnoteIndex, stripFootnoteRef } from "../utils/footnote-ids";
...
async function handleSimilarIssuesAndComments(
  ...
  let footnoteIndex = nextFootnoteIndex(commentBody);
...
export function removeAnnotateFootnotes(content: string): string {
  ...
  if (footnotes) {
    footnotes.forEach((footnote) => {
      const footnoteRef = footnote.match(/\[\^((?:annotation-)?\d+)\^\]/)?.[0];
      if (footnoteRef) {
