...
export async function issueDedupe(context: Context<"issues.opened" | "issues.edited">, options: { keepUpdateComment?: boolean } = {}) {
  ...
  if (similarIssues && similarIssues.length > 0) {
    ...
    if (matchIssues.length > 0) {
      ...
      const bestMatchIssue = getBestMatchIssue(matchIssues);
      ...
      if (shouldClose) {
        await closeIssueAsDuplicate(context, originalIssue.node_id, bestMatchIssue.node.id);
      }
      return;
    }
...
function getBestMatchIssue(matchIssues: IssueGraphqlResponse[]): IssueGraphqlResponse {
  return [...matchIssues].sort((a, b) => parseFloat(b.similarity) - parseFloat(a.similarity))[0];
}
...
async function closeIssueAsDuplicate(context: Context, issueId: string, duplicateIssueId: string) {
  await context.octokit.graphql(
    /* GraphQL */
    `
      mutation CloseDuplicateIssue($issueId: ID!, $duplicateIssueId: ID!) {
